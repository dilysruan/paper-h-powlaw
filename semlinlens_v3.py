...
class lens:
    ...
    def procparms(self,eta,b_in,e_in,h_in):
        if self.Amode=='b':
            b_out = b_in
        elif self.Amode=='Rein':
            btmp = max(b_in,0.01)
            efac = max(1.0-0.5*e_in,0.01)
            b_out = (btmp*efac)**(2.0-eta)
        else:
    ...
    def lnP_circ(self,p):
        ...
        htmp = p[1]
        etmp = 0.0
        b,h = self.procparms(eta,btmp,etmp,htmp)
    ...
    def lnP_fullsum1(self,pall):
        ...
        # CRK new 2023/8/24: compute time zeropoint in case it's not 0
        T0 = np.sum((tdel-xi)/self.tsig**2)/np.sum(1/self.tsig**2)
        tdel -= T0
        ...
        # CRK updated 2023/8/24: now that we account for T0 as above, we should include all terms
        chisq += np.sum((tdel-xi)**2/self.tsig**2)
        # for i in range(1,self.nimg):
        #     dt = tdel[i] - xi[i]
        #     chisq += dt**2/self.tsig[i]**2
        
        # CRK 2023/8/24: do *not* want the extra chisq here
        # chisq += chisq + (eta-1.0)**2/(0.1)**2
        chisq += (eta-1.0)**2/(0.1)**2
    ...
    def opt_MCin(self, nopt, basename,outname,zlens, zsrc, ellip=False,eta_range=[0.5,1.5], mode='fullsum1', fluxmode='none'):
        ...
        for iran in range(nopt):
            ...
            if self.Amode=='b':
                etmp = np.sqrt(ec**2+es**2)
                b = (ravg*(1.0-0.5*etmp))**(2.0-eta)
            elif self.Amode=='Rein':
                b = ravg
            else:
                print('ERROR: unrecognized value of Amode')
                return
            # always guess h = 0.7
            if self.hmode == 'hinv':
                h = 1/0.7
